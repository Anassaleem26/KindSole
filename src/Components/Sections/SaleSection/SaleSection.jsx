...
import saleBanner from '../../../assets/Sale section/Sale Section.png';
...
function SaleSection() {
    ...
    useEffect(() => {
        (async () => {
            try {
                setIsLoading(true)
                setError(null)
                const fetchedProducts = await configservice.getAllProducts();
                if (fetchedProducts) {
    ...
    return (
        <div>
            <div className="relative h-135 pb-22 w-full flex justify-center items-end  ">
                <div
                ...
                ></div>
                <div className="relative z-10 flex justify-center items-center  pl-19 gap-5 perspective-[800px] ">

                    <Link to="/salesection/mensale">
                        <Button
                            className='uppercase bg-[#5b3f3f] text-white rounded-[20px] text-[17px] opacity-90 cursor-pointer transition-transform duration-150 transform-gpu preserve-3d active:translate-z-[-10px] active:scale-97 hover:scale-105 '
                        >
                            Men's Sale
                        </Button>
